...
- `.github/workflows/deploy-monsterasp.yml`: optional automated deployment (manual `workflow_dispatch` trigger) to MonsterASP via GitHub Actions — pins .NET SDK 8 precisely (avoids the local SDK 10 bug), builds Angular, runs tests, then deploys via `rasmusbuchholdt/simply-web-deploy` (instead of MSBuild's broken built-in WebDeploy). Requires 4 GitHub secrets the user adds themselves (documented in `DEPLOY-MonsterASP-AR.md`).
