...
- **FTP (الأسهل مع FileZilla):** اتصل ببيانات FTP من الخطوة 1، وارفع **محتوى** مجلد `dist\MonsterASP-Package\` (وليس المجلد نفسه) إلى المجلد الجذر لموقعك على الخادم (عادة `wwwroot` أو الجذر مباشرة حسب تسمية اللوحة).
- **File Manager:** لو اللوحة تدعم رفع وفكّ ضغط ZIP مباشرة، ارفع `dist\MonsterASP-Package.zip` وفكّ ضغطه في مجلد الموقع.
